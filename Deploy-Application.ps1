...
Try {
    ...
    [String]$appName = 'Microsoft Access 2016 Runtime'
    [String]$appVersion = ''
    [String]$appArch = ''
    [String]$appLang = 'DE'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        Show-InstallationWelcome -CloseApps 'msaccess' -AllowDefer -DeferTimes 3 -CheckDiskSpace -PersistPrompt
		...
		Execute-Process -Path "$dirFiles\setup.exe" -Parameters "/config ""$dirFiles\Access2016RT.xml""" -WindowStyle Hidden
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        Show-InstallationWelcome -CloseApps 'msaccess' #-CloseAppsCountdown 60
		...
		# Uninstall Any Existing Version of Microsoft Access Runtime 2016
		$SourceCache = Get-InstalledApplication -Name 'Microsoft Access Runtime 2016'
        
        ForEach ($item in $SourceCache) {
			If($item.UninstallString) {
				$UninstPath = $item.UninstallString -replace '"', '' -replace '/uninstall ACCESSRT /dll OSETUP.DLL', ''
				
				If(Test-Path -Path $UninstPath) {
					Execute-Process -Path $UninstPath -Parameters "/uninstall ""AccessRT"" /config ""$dirFiles\Access2016RT.xml"""
				}
			}
        }
